...
from deeplearning.datasets.utils.normalize import normalize_image, ct_normalize_image, mr_normalize_image
...
def ct_collate_fn_tr_styleaug_SABAug(batch):
    image, label, name = zip(*batch)
    image = np.stack(image, 0)
    label = np.stack(label, 0)
    name = np.stack(name, 0)
    ...
    data_dict = tr_transforms(**data_dict)
    fda_data = fourier_augmentation_reverse(data_dict['data'])
    data_dict['fda_data'] = ct_normalize_image(fda_data)
    GLA, LLA = sl_augmentation(data_dict['data'], data_dict['seg'])
    SABA = brightness_augmentation(data_dict['data'], data_dict['seg'])
    data_dict['data'] = ct_normalize_image(data_dict['data'])
    data_dict['GLA'] = ct_normalize_image(GLA)
    data_dict['LLA'] = ct_normalize_image(LLA)
    data_dict['SABA'] = ct_normalize_image(SABA)
    return data_dict

def mr_collate_fn_tr_styleaug_SABAug(batch):
    image, label, name = zip(*batch)
    image = np.stack(image, 0)
    label = np.stack(label, 0)
    name = np.stack(name, 0)
    ...
    data_dict = tr_transforms(**data_dict)
    fda_data = fourier_augmentation_reverse(data_dict['data'])
    data_dict['fda_data'] = mr_normalize_image(fda_data)
    GLA, LLA = sl_augmentation(data_dict['data'], data_dict['seg'])
    SABA = brightness_augmentation(data_dict['data'], data_dict['seg'])
    data_dict['data'] = mr_normalize_image(data_dict['data'])
    data_dict['GLA'] = mr_normalize_image(GLA)
    data_dict['LLA'] = mr_normalize_image(LLA)
    data_dict['SABA'] = mr_normalize_image(SABA)
    return data_dict