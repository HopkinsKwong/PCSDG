...
from deeplearning.models.unet_pcsdg_simple import *
...
from deeplearning.datasets.utils.transform import collate_fn_tr_styleaug, collate_fn_ts, collate_fn_tr_styleaug_SABAug
from deeplearning.utils.lr import adjust_learning_rate
from deeplearning.utils.metrics.dice import get_hard_dice
...
def train(args):
    ...
    tr_dataloader = torch.utils.data.DataLoader(tr_dataset,
                                                ...
                                                shuffle=shuffle,
                                                pin_memory=True,
                                                collate_fn=collate_fn_tr_styleaug_SABAug)
    ...
    parameters_to_update = list(projector.parameters()) + list(model.DWM.parameters())
    optimizer_prompt = torch.optim.SGD(parameters_to_update,
    ...
    print('start epoch: {} - PCSDG_simple + SABA'.format(start_epoch))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for iter, batch in enumerate(tr_dataloader):
            ...
            SABA_data = torch.from_numpy(batch['SABA']).cuda().to(dtype=torch.float32)
            seg = torch.from_numpy(batch['seg']).cuda().to(dtype=torch.float32)

            optimizer_prompt.zero_grad()
            with autocast():
                f_content, f_style = model.forward_DisentangleWeightMap(data)
                f_content_fda, f_style_fda = model.forward_DisentangleWeightMap(fda_data)
                f_content_GLA, f_style_GLA = model.forward_DisentangleWeightMap(GLA_data)
                f_content_SABA, f_style_SABA = model.forward_DisentangleWeightMap(SABA_data)
                ...
                f_content_SABA_p = projector(f_content_SABA)
                f_style_SABA_p = projector(f_style_SABA)
                ...
                content_loss = F.l1_loss(f_content_p, f_content_fda_p, reduction='mean') + \
                               ...
                               F.l1_loss(f_content_p, f_content_SABA_p, reduction='mean') + \
                               F.l1_loss(f_content_SABA_p, f_content_p, reduction='mean') + \
                               F.l1_loss(f_content_fda_p, f_content_SABA_p, reduction='mean') + \
                               F.l1_loss(f_content_SABA_p, f_content_fda_p, reduction='mean') + \
                               F.l1_loss(f_content_GLA_p, f_content_SABA_p, reduction='mean') + \
                               F.l1_loss(f_content_SABA_p, f_content_GLA_p, reduction='mean')
                ...
                style_loss = F.l1_loss(f_style_p, f_style_fda_p, reduction='mean') + \
                             ...
                             F.l1_loss(f_style_p, f_style_SABA_p, reduction='mean') + \
                             F.l1_loss(f_style_SABA_p, f_style_p, reduction='mean') + \
                             F.l1_loss(f_style_fda_p, f_style_SABA_p, reduction='mean') + \
                             F.l1_loss(f_style_SABA_p, f_style_fda_p, reduction='mean') + \
                             F.l1_loss(f_style_GLA_p, f_style_SABA_p, reduction='mean') + \
                             F.l1_loss(f_style_SABA_p, f_style_GLA_p, reduction='mean')
    ...
    # inference
    from deeplearning.inference.inference_nets.inference_unet_pcsdg_simple import inference
